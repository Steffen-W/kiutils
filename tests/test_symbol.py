...
class Tests_Symbol_Since_V9(unittest.TestCase):
    ...
    def test_v9HidePropertiesFormatting(self):
        ...
        self.testData.pathToTestFile = path.join(SYMBOL_BASE, 'since_v9', 'test_v9_hide_properties')
        symbolLib = SymbolLib().from_file(self.testData.pathToTestFile)
        ...
        visibleProps = [p.key for p in symbol.properties if not (p.effects and p.effects.hide)]
    ...
    def test_v9NewPropertiesParsing(self):
        ...
        self.assertFalse(symbol.excludeFromSim, "exclude_from_sim should be False (value 'no')")

        # Test serialization includes new properties
        serialized = symbolLib.to_sexpr()
        self.assertIn("exclude_from_sim no", serialized, "exclude_from_sim should be in output")
        self.assertIn("embedded_fonts no", serialized, "embedded_fonts should be in output")
        ...
        self.assertEqual(symbol.excludeFromSim, reparsedSymbol.excludeFromSim, "exclude_from_sim should be preserved")
    ...
    def test_v9ComprehensiveFeatures(self):
        ...
        self.assertEqual(symbolLib.generator, "kicad_symbol_editor", "Should parse v9 generator")
        ...
        visibleCount = sum(1 for p in symbol.properties if not (p.effects and p.effects.hide))
        ...
        propStatus = {p.key: (p.effects.hide if p.effects else False) for p in symbol.properties}
        
        # These should be visible
        self.assertFalse(propStatus.get("Reference", True), "Reference should be visible")
        self.assertFalse(propStatus.get("Value", True), "Value should be visible")
        
        # These should be hidden
        self.assertTrue(propStatus.get("Footprint", False), "Footprint should be hidden")
        self.assertTrue(propStatus.get("Datasheet", False), "Datasheet should be hidden")
        self.assertTrue(propStatus.get("Description", False), "Description should be hidden")
    ...
    def test_v9ToV8ConversionWorkflow(self):
        ...
        originalHiddenCount = sum(1 for p in originalSymbol.properties if p.effects and p.effects.hide)
        ...
        convertedHiddenCount = sum(1 for p in convertedSymbol.properties if p.effects and p.effects.hide)
        
        self.assertEqual(originalHiddenCount, convertedHiddenCount, "Hidden property count should be preserved")
        self.assertEqual(originalPinHide, convertedSymbol.pinNamesHide, "pin_names hide should be preserved")
        self.assertEqual(originalExcludeSim, convertedSymbol.excludeFromSim, "exclude_from_sim should be preserved")
        
        # Test v8 format output attributes
        self.assertEqual(convertedLib.version, "20211014", "Should output v8 version")
        self.assertEqual(convertedLib.generator, "kiutil", "Should output kiutil generator")
    ...
    def test_v9PinNamesAndPinNumbersHideParsing(self):
        ...
        self.assertTrue(symbol.pinNames, "pin_names should be enabled")
        self.assertTrue(symbol.pinNamesHide, "pin_names hide should be parsed from (hide yes)")
        
        # Test that offset parsing still works alongside hide
        if symbol.pinNamesOffset is not None:
            self.assertIsInstance(symbol.pinNamesOffset, (int, float), "pin_names offset should be numeric")
        ...
        self.assertEqual(hideResults, expectedResults, "Hide property parsing should handle all formats")